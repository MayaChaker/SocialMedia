export default function ProductVisual({ type, product }) {
  const generatedImage = product?.image || ({ tube: "/products/veloura-cleanser.webp", bottle: "/products/veloura-serum.webp", serum: "/products/veloura-serum-bestseller-v2.webp", lipstick: "/products/veloura-lipstick.webp", jar: "/products/veloura-jar.webp", compact: "/products/veloura-compact.webp", set: "/products/veloura-set.webp", duo: "/products/veloura-set.webp" }[type]);
  if (generatedImage) return <img className="realProductImage" src={generatedImage} alt={`${product?.name || "Veloura beauty product"}${product?.selectedVariant ? ` in ${product.selectedVariant}` : ""}`} loading="lazy" decoding="async"/>;
